class Auto
{
    ...
    public void move(int distance) //езда
    {
        ...
        {
            Console.WriteLine("Недостаточно топлива для дальнейшей поездки.");
            Console.WriteLine("Желаете заправиться?? \n" +
                              "Нажмите - 1 если Да\n" +
                              "Нажмите - 2 если Нет");
            ...
            {
                this.braking();
                Console.WriteLine("Поездка окончена");            
            }
        }
    }
...
class Program
{
    static void Main(string[] args)
    {
        ...
        while (true) // получаем случайное расстояние
        {
            ...
            if (answer == "1") // Прерывание езды если решим закончить поезку
            {
                goto restart;
            }
            else if (answer == "2")
        ...
        Console.WriteLine("Весь пробег: " + car.mileag(0, 0, 0, 0)); // Общий пробег авто        
    }
}